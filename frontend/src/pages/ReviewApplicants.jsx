...
const ReviewApplicants = () => {
  ...
  const fetchApplicants = async () => {
    ...
    try {
      ...
      const inReviewApplicants = response.content.filter(a => a.status === 'IN_REVIEW');
      setApplicants(inReviewApplicants);
    } catch (error) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setSelectedApplicant(null);
      showAlert('Applicant has been rejected', 'success');
    } catch (error) {
      console.error('Error rejecting applicant:', error);
      showAlert('Failed to reject applicant', 'error');
    } finally {
      setUpdatingStatus(false);
    }
  };
        ...
        <header className="bg-white shadow-sm sticky top-0 z-10 border-b">
          <div className="px-8 py-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Review Applicants</h1>
                <p className="text-sm text-gray-500 mt-1">
                  Applicants currently in review status - {filteredApplicants.length} total
                </p>
              </div>
        ...
        <main className="p-8">
          ...
          {filteredApplicants.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No Applicants in Review</h3>
              <p className="text-gray-500">There are currently no applicants with IN_REVIEW status</p>
            </div>
          ) : (
      ...
      {showModal && selectedApplicant && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-4xl w-full shadow-2xl">
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6 flex items-center justify-between rounded-t-xl">
              <div className="flex items-center space-x-4">
                <div className="w-16 h-16 rounded-xl bg-white text-blue-600 flex items-center justify-center font-bold text-2xl shadow-lg">
